...
- ConsumerNetworkClient → 내부 버퍼 Linked Queue 적재: 비동기
- Fetcher가 내부 버퍼 Linked Queue 에서 꺼내서 poll() 반환: 동기

~~~
[브로커] → (비동기) → ConsumerNetworkClient → 내부 큐
                                         ↓
                                  (동기)
                                     ↓
                                  Fetcher → poll()
...
- Linked Queue (정확히는 내부 메모리 buffer!)
- Heap 메모리. JVM Heap에 올라감. 
- Consumer 인스턴스 단위 (JVM 내부). VM 단위 아님.
- consumer 하나 생성하면 내부에 각각 존재. 즉, 서로 동유 안함.
- **consumer 가 느리면, 내부 queue 에 계속 쌓일 것. 그럼 메모리 압박이 큼. 그리고 eventually rebalance 발생함.
    - 그래서 해당 config 제어를 잘 해서 메시지 소비를 해야 함.**
...
- enable.auto.commit : true 자동 저장

### ❓ __consumer_offsets

- 카프카의 내부 토픽. Kafka Broker의 로그 디렉토리 (디스크)
- 클러스터 replication 가능 
- 다음에 읽을 offset 정보를 알 수 있다.
- consumer 단위가 아닌, consumer group 단위임.
...
- 별도의 thread
- 데이터를 잘 소비해도 heartbeat 문제 생기면 죽었다고 판단.
~~~
session.timeout.ms 초과 → Coordinator가 죽었다고 판단 → Rebalance
~~~
...
### 리밸랜싱
- Broker의 Group Coordinator는 heartbeat를 기준으로만 생존을 판단한다.
- 1) polling 도 잘하고, 2) heartbeat 도 잘 보내야 살아 있음. commit 여부 상관없음.
  - 이때, polling 자체로 브로커 coordinator 가 판단하는 건 아니고, polling 을 안 하면 consumer 에서 스스로 heatbeat 를 중단함. 그래서 리밸런싱을 일으킨다.
  - Coordinator는 polling 여부를 직접 알지 못한다.
- heatbeat thread 와 polling 하는 main thread 는 별도임.
- polling을 안 하면 Consumer 내부에서 max.poll.interval을 초과했다고 판단하고,
  스스로 heartbeat를 중단(또는 LeaveGroup)한다.
- 그 결과 heartbeat가 끊기고,
  Coordinator는 해당 Consumer를 죽었다고 판단하여 rebalance를 발생시킨다.
- commit 여부는 rebalance 판단과 무관하다.
- heartbeat thread와 poll을 수행하는 main thread는 별도로 동작한다

1) polling
   - max.poll.interval.ms 초과 → Rebalance
2) heartbeat
  - session.timeout.ms 초과 → Rebalance
...
2) polling
 → poll 시간 체크
 → max.poll.interval 지나면, consumer 는 스스로 죽음 (heartbeat 중단 or LeaveGroup)
...
### Coordinator는 “poll 안 하는 걸 어떻게 아냐?”
- 사실 모름. 그러나,
- consumer 는 내부적으로 마지막 poll() 호출 시간을 기록함.
  - KafkaConsumer 내부 상태 (클라이언트 메모리. JVM Heap). consumer 인스턴스 단위.
  - 브로커는 poll이 얼마나 걸리는지 모름. 그래서 브로커는 heartbeat 기준으로 아는 것.
- consumer 는 내부적으로 max.poll.interval.ms 를 기준으로, 이 이상이 넘어가면 poll 을 오래 안했다고 판단함. => heatbeat 중단. leave group => Coordinator heatbeat 안 오기에 리밸런싱.
